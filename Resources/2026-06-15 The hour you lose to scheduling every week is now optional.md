...
Newsletter Marketing AI Playbook opisuje integrację [[Buffer]] z [[Claude]] przez MCP, która pozwala planować tygodniowy kalendarz postów w social mediach w około 20 minut zamiast godziny. Workflow polega na wklejeniu jednej mocnej treści (newsletter, transkrypt, długi post) i poproszeniu Claude'a o wyciągnięcie 5 pomysłów i napisanie gotowych postów na X, Threads i LinkedIn. Kluczowa teza: dystrybucja treści to pierwszy system, który się sypie gdy jedna osoba prowadzi wszystko — i właśnie to warto automatyzować. Integracja działa tylko na platformach tekstowych; Instagram wymaga ręcznego dodania wizualiów.

## Frameworki i metody

- **Jednorazowy setup Buffer MCP** — w [[Claude]] (Customize → Connectors → Add custom connector) wkleić `mcp.buffer.com/mcp` i zalogować się; wcześniej połączyć kanały w [[Buffer]]; całość zajmuje ~5 minut
...
- [[Buffer]] jako MCP connector do [[Claude]] eliminuje ręczny etap kopiowania i ustawiania harmonogramu — zatwierdzasz treść, nie obsługujesz narzędzia
