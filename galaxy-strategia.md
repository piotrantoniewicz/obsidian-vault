# Strategia rozbudowy Galaxy/ — 2026-06-30 (przegląd re-source zakończony)
...
**Ostatnia operacja — przegląd re-source ZAKOŃCZONY (2026-06-30):** jednorazowy przegląd **wszystkich 21 stron** Galaxy/ nowym workflow (3 kanały recall) domknięty — każda strona ma przeliczone źródła, dorzucone nowe/świeższe i zaktualizowaną treść + `updated`. Tymczasowa sekcja „Status jednorazowego przeglądu…" **usunięta z `CLAUDE.md`** zgodnie z regułą samo-usunięcia (trwałe sekcje „Uzgadnianie liczników…" i „Walidacja wikilinków…" zostają). Galaxy = 21 stron (bez nowych — re-source nie tworzy pojęć). Ostatnia tura (6): AI governance — EU AI Act Digital Omnibus (compliance high-risk → 2 XII 2027, Art. 25, AI Office) + odpowiedzialność za każdy output AI i 3-etapowy proces (Fundraising Regulator UK); Context engineering — nauka-przez-robienie i „Personal OS" (Raviv), koszt okna pliku pamięci AGENTS.md; Prompt engineering — meta-prompt procesowy „najpierw wywiad" (Carr), narzędzia Claude Prompt Improver/ChatGPT Optimizer i „zrozum proces zamiast list 100 promptów" (Śliwowski), sygnał deprecjacji mikrooptymalizacji w modelach rozumujących. Wcześniejsze tury: RAG (przyczyna halucynacji + „bezpieczny ≠ skuteczny"), Agentic AI (teoria agencji/moral hazard, case'y NGO), Widoczność w AI search (3 filary GEO, Big Star/LLMwatcher), Owned vs rented (Krikorian, cozy Internet), Storytelling (Canva, „content dictates structure"), Newsletter (advocacy ETUC, „powody nie rabaty"), Email deliverability (sygnał bezpośredni vs pochodny), Framing (peak-end rule, Łatwogang), P2P (~2× crowdfunding, Garkowski), Wdrażanie AI (readiness first, Crawl-Walk-Run) oraz 7 stron fundraisingowych. **Czerwone linki ≥2 do napisania** (próg osiągnięty): RODO i dane wrażliwe; Transfer międzypokoleniowy majątku. Żadna tura przeglądu nie zmieniała sekcji „Powiązane pojęcia" → liczniki czerwonych linków bez zmian (pozostałe po 1: Public Narrative, Wizualizacja danych, Storytelling w jednym zdaniu, Suwerenność technologiczna, Mobilizacja cyfrowa, LLM Wiki, Sprawczość organizacyjna, Harness i scaffolding, Claude Projects). **Cleanup 2026-06-30:** usunięto duplikat „2026-03-19 Poza feedem…?" (re-clip) — Resources + Archives + wpis w index; zachowano wersję bez „?".

**Następny krok:** przegląd re-source domknięty — Galaxy wraca do trybu **wzrostu od popytu** (zasada nadrzędna niżej). Priorytet: napisać dwie strony z przekroczonym progiem ≥2 (RODO i dane wrażliwe; Transfer międzypokoleniowy majątku), potem klastry-kandydaci (Evale, Context layer organizacji).
